...
func InjectRoutes(router *mux.Router, grpcClient adapter.GrpcClient, amqpClient *adapter.AmqpClient, redisClient adapter.RedisClient) {
	...
	subRouterBook.Path("/{book_uuid:[a-zA-Z0-9]{24}}/image/url").HandlerFunc(controller.GetBookPresignUrl(amqpClient, redisClient)).Methods(http.MethodGet)
	subRouterBook.Path("/{book_uuid:[a-zA-Z0-9]{24}}").HandlerFunc(controller.UpdateBook(amqpClient)).Methods(http.MethodPut)
	subRouterBook.Path("/{book_uuid:[a-zA-Z0-9]{24}}/image").HandlerFunc(controller.UpdateBookWithImage(amqpClient, redisClient)).Methods(http.MethodPut)
